...
--1. EMP 테이블에서 인원수,최대 급여,최소 급여,급여의 합을 계산하여 출력하는 SELECT 문장을 작성하여라.
  select count(*) "인원 수", max(sal) "최대 급여", min(sal) "최소 급여", sum(sal) "급여의 합"
    from emp;

--2. EMP 테이블에서 각 업무별로 최대 급여,최소 급여,급여의 합을 출력하는 SELECT 문장을 작성하여라.
  select job "업무", max(sal) "최대 급여", min(sal) "최소 급여", sum(sal) "급여의 합"
    from emp
group by job;
...
--4. EMP 테이블에서 최고 급여와 최소 급여의 차이는 얼마인가 출력하는 SELECT 문장을 작성하여라.
  select max(sal) - min(sal) "최고 급여와 최소 급여의 차이"
    from emp;
--5. EMP 테이블에서 부서별로 인원수, 평균 급여, 최저급여, 최고 급여, 급여의 합을 구하여 출력하여라.
  select deptno "부서", count(*) "인원 수", avg(sal) "평균 급여", max(sal) "최대 급여", min(sal) "최소 급여", sum(sal) "급여의 합"
    from emp
group by deptno;
...
--7. 부서별, 업무별 그룹하여 결과를 부서번호, 업무, 인원수, 급여의 평균, 급여의 합을 구하여 출력하여라. 
  select deptno "부서번호", job "업무", count(*) "인원 수", avg(sal) "급여의 평균", sum(sal) "급여의 합"
    from emp
group by deptno, job;

--8. 업무별, 부서별 그룹하여 결과를 부서번호, 업무, 인원수, 급여의 평균, 급여의 합을 구하여 출력하여라.
 select job "업무", deptno "부서번호", count(*) "인원 수", avg(sal) "급여의 평균", sum(sal) "급여의 합"
    from emp
group by job, deptno;
--9. EMP 테이블에서 급여가 최대 2900이상인 부서에 대해서 부서번호, 평균 급여, 급여의 합을 구하여 출력하여라.
  select deptno "부서번호", avg(sal) "평균 급여", sum(sal) "급여의 합"
    from emp
group by deptno
  having max(sal) >= 2900;
...
--12. EMP 테이블에서 부서 인원이 4명보다 많은 부서의 부서번호, 인원수, 급여의 합을 구하여 출력하라
  select deptno "부서번호", count(*) "인원 수", sum(sal) "급여의 합"
    from emp
group by deptno
  having count(*) > 4;
